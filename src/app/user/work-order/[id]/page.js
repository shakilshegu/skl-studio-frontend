...
import Initiation from "../../../../components/BookIduser/Initiation.jsx";
...
import { getBookingDetails } from "@/services/Booking/user.bookings.service.js";
import { getTicketByBookingId } from "@/services/support/support.service.js";
import SupportTicketsModal from "../../../../components/Support/SupportTicketsModal.jsx";
export default function OrderTracking() {
  ...
  const getStorageKey = () => `userOrderTracking_activeStep_${id}`;
  ...
  const {
    ...
    queryFn: () => getTicketByBookingId(id),
  ...
  useEffect(() => {
    if (bookingData?.workFlowStatus) {
      ...
      const workflowStep = getStepFromWorkflowStatus(
        bookingData.workFlowStatus
      );

      // Debug logging
      if (process.env.NODE_ENV === "development") {
        console.log("🔄 User Step Resolution:", {
          currentActiveStep: activeStep,
          workflowStatus: bookingData.workFlowStatus,
          workflowStep,
          savedStep,
  ...
  const handleSetActiveStep = (step) => {
    if (process.env.NODE_ENV === "development") {
      console.log(`🎯 Setting user active step: ${step} (was: ${activeStep})`);
    }
    ...
    if (typeof window !== "undefined") {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(`💾 Saved to localStorage: ${getStorageKey()} = ${step}`);
      }
    }
  };
  ...
  const isStepAccessible = (stepId) => {
    ...
    const currentWorkflowStep = getStepFromWorkflowStatus(
      bookingData.workFlowStatus
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-indigo-50 p-6">
      <div className=" mx-auto space-y-6">
          ...
          <div className="bg-white px-8 py-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                ...
                <button
                  className="flex items-center space-x-2 bg-orange-500 hover:bg-orange-600 px-4 py-2 rounded-xl text-white font-medium transition-colors"
        ...
        {/* Progress Stepper */}
        <div className="bg-transparent px-8 py-3">
          {/* Desktop Stepper */}
                ...
                {steps.map((step, index) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        isAccessible && handleSetActiveStep(step.id)
                      }
                    >
                      {/* Step Circle */}
                      <div
        ...
        {/* Content Section */}
        <div>{steps.find((step) => step.id === activeStep)?.content}</div>
      </div>
      <SupportTicketsModal
