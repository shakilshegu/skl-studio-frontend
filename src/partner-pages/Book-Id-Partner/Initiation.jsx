...
import { Calendar, Clock, User, Phone, Mail, Package, Wrench, Users, Briefcase } from "lucide-react";
import moment from "moment";

const tabs = ["Services", "Packages", "Equipments", "Helpers"];

export default function PartnerInitiation({role, bookingData, setActiveStep}) {
    ...
    const userData = bookingData?.userId || {};
    const { name, mobile, email, profile } = userData;
    ...
    const getTabIcon = (tabName) => {
        switch(tabName) {
            ...
            case 'Equipments': return <Wrench size={16} />;
            case 'Helpers': return <Users size={16} />;
            default: return null;
        }
    };
    ...
    const getTabCount = (tabName) => {
        switch(tabName) {
            ...
            case 'Packages': return packages?.length || 0;
            case 'Equipments': return equipments?.length || 0;
            case 'Helpers': return helpers?.length || 0;
            default: return 0;
        }
    };
                ...
                <div className="bg-white/20 p-3 rounded-xl">
                                ...
                                <div>
                                    ...
                                    <p className="text-white">
                                        Review your booking details and requirements
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center space-x-4">
                                <div className="bg-white/20 rounded-xl px-4 py-2">
                                    ...
                                    <div className="text-white text-sm">Booking ID</div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="flex items-center space-x-6">
                            <div className="relative">
                                {profile?.profilePhoto ? (
                                    ...
                                    <div className="w-20 h-20 bg-[#892580] rounded-full flex items-center justify-center shadow-lg">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-gray-800 mb-2">{name || 'N/A'}</h3>
                                <div className="space-y-2">
                                    <div className="flex items-center space-x-2 text-gray-600">
                                        <Phone size={16} className="text-[#892580]" />
                                        <span className="text-sm">{mobile || 'N/A'}</span>
                                    </div>
                                    <div className="flex items-center space-x-2 text-gray-600">
                            ...
                            <div className="space-y-3">
                                {Array.isArray(dates) &&
                                    [...dates] // clone to avoid mutating original
                                        ...
                                        .map((dateObj, index) => {
                                            const formattedDate = moment(dateObj.date).format("MMMM Do YYYY, dddd");
                                            const start = moment({ hour: dateObj.startTime }).format("hh:mm A");
                                            const end = moment({ hour: dateObj.endTime }).format("hh:mm A");
                                            const duration = dateObj.endTime - dateObj.startTime;
                                            const hoursText = `${duration}hr${duration > 1 ? 's' : ''}`;

                                            return (
                                                <div key={index} className="bg-gradient-to-r from-purple-50 to-blue-50 p-4 rounded-xl border border-purple-100">
                            ...
                            <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-6">
                                {services?.length > 0 ? (
                                    ...
                                    <div className="col-span-full text-center py-12">
                                        ...
                                        <h3 className="text-lg font-semibold text-gray-700 mb-2">No Services</h3>
                                        <p className="text-gray-500">No services have been added to this booking.</p>
                                    </div>
                                )}
                            </div>
                        )}
                            ...
                            <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-6">
                                {packages.length > 0 ? (
                                    ...
                                    <div className="col-span-full text-center py-12">
                                        ...
                                        <h3 className="text-lg font-semibold text-gray-700 mb-2">No Packages</h3>
                                        <p className="text-gray-500">No packages have been added to this booking.</p>
                                    </div>
                                )}
                            </div>
                        )}
                            ...
                            <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-6">
                                {equipments.length > 0 ? (
                                    ...
                                    <div className="col-span-full text-center py-12">
                                        ...
                                        <h3 className="text-lg font-semibold text-gray-700 mb-2">No Equipment</h3>
                                        <p className="text-gray-500">No equipment has been added to this booking.</p>
                                    </div>
                                )}
                            </div>
                        )}
                            ...
                            <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-6">
                                {helpers.length > 0 ? (
                                    helpers.map((helper) => (
                                        ...
                                        >
                                            <div className="p-6">
                                                <div className="w-16 h-16 bg-[#892580] rounded-xl mb-4 flex items-center justify-center">
                                    ...
                                    <div className="col-span-full text-center py-12">
                                        ...
                                        <h3 className="text-lg font-semibold text-gray-700 mb-2">No Helpers</h3>
                                        <p className="text-gray-500">No helpers have been added to this booking.</p>
                                    </div>
                                )}
                            </div>
                        )}
